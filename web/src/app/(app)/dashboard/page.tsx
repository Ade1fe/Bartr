      ...
      <section className='max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-8 space-y-6 md:space-y-8'>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          <StatCard
            icon={<Box size={18} />}
            label="Active Trades"
            value={data?.stats.activeTrades}
            delta={data?.stats.activeTradesDelta}
            loading={isLoading}
          />
          <StatCard
            icon={<Coins size={18} />}
            ...
            delta={data?.stats.creditBalanceDelta}
            loading={isLoading}
          />
          <StatCard
            icon={<CheckCircle2 size={18} />}
            label="Completed"
            value={data?.stats.completedTrades}
            delta="All time"
            loading={isLoading}
          />
          <StatCard
            icon={<Star size={18} />}
            label="Reputation"
            value={data?.stats.reputation.toFixed(1)}
            delta={`Based on ${data?.stats.reviewCount ?? 0} reviews`}
            loading={isLoading}
          />
        </div>
